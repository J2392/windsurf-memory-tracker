...
from PyQt6.QtCore import (
    Qt, QSize, QDateTime, QTimer, QRegularExpression, 
    pyqtSignal, QMimeData, QPoint
)
from PyQt6.QtGui import (
    QTextCharFormat, QColor, QSyntaxHighlighter, 
    QTextCursor, QPixmap, QDrag
)
...
from ultis import logger, format_time_ago
...
class TaskCard(QFrame):
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() & Qt.MouseButton.LeftButton:
            ...
            logger.debug(f"Manhattan length: {manhattan_length}, start drag distance: {start_drag_distance}")
            
            if manhattan_length < start_drag_distance:
                return
            
            logger.debug(f"Bắt đầu kéo thả task {self.task_id}")
            ...
            logger.debug(f"Đã tạo mime data cho task {self.task_id}")
            logger.debug(f"MIME formats: {mime_data.formats()}")
            ...
            logger.debug(f"Đã tạo drag object cho task {self.task_id}")
            
            # Thực hiện kéo
            logger.debug(f"Thực hiện kéo task {self.task_id}")
            result = drag.exec(Qt.DropAction.MoveAction)
            
            # Log kết quả kéo thả
            if result == Qt.DropAction.MoveAction:
                logger.info(f"Kéo thả thành công task {self.task_id}")
            else:
                logger.info(f"Kéo thả không thành công task {self.task_id}, kết quả: {result}")
...
class KanbanColumn(QWidget):
    ...
    def add_new_task(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            logger.info(f"Tạo task mới {task_data['id']} trong cột {self.title} ({self.status})")
    ...
    def on_task_moved(self, task_id: str, new_status: str):
        ...
        # Truyền sự kiện lên parent widget
        logger.debug(f"KanbanColumn: Phát tín hiệu di chuyển task {task_id} sang {new_status}")
    ...
    def dragEnterEvent(self, event):
        ...
        logger.debug(f"dragEnterEvent cho cột {self.title}")
        logger.debug(f"MIME formats: {event.mimeData().formats()}")
        
        if event.mimeData().hasText():
            logger.debug(f"MIME text: {event.mimeData().text()}")
            
        if event.mimeData().hasFormat("application/x-task"):
            task_id = event.mimeData().data("application/x-task").data().decode()
            logger.debug(f"Chấp nhận kéo task {task_id} vào cột {self.title}")
            event.setDropAction(Qt.DropAction.MoveAction)
            event.accept()
        else:
            logger.debug(f"Từ chối kéo vào cột {self.title} (không phải task)")
            # Thử kiểm tra nếu có text
            if event.mimeData().hasText():
                task_id = event.mimeData().text()
                logger.debug(f"Nhưng có text: {task_id}, thử chấp nhận")
                event.setDropAction(Qt.DropAction.MoveAction)
                event.accept()
            else:
                event.ignore()
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().hasFormat("application/x-task"):
            task_id = event.mimeData().data("application/x-task").data().decode()
            logger.debug(f"Nhận được task {task_id} thả vào cột {self.title} từ MIME data")
        elif event.mimeData().hasText():
            task_id = event.mimeData().text()
            logger.debug(f"Nhận được task {task_id} thả vào cột {self.title} từ text")
        ...
        if task_id:
            ...
            for column in self.parent().findChildren(KanbanColumn):
                for i in range(column.tasks_layout.count()):
                    widget = column.tasks_layout.itemAt(i).widget()
                    if isinstance(widget, TaskCard) and widget.task_id == task_id:
                        source_card = widget
            ...
            if source_card:
                logger.debug(f"Tìm thấy source card cho task {task_id} ở cột {source_column.title} ({source_column.status})")
                
                if source_column.status != self.status:
                    # Di chuyển task sang cột mới
                    logger.info(f"Kéo thả task {task_id} từ cột {source_column.title} ({source_column.status}) sang cột {self.title} ({self.status})")
                    self.taskMoved.emit(task_id, self.status)
                    event.setDropAction(Qt.DropAction.MoveAction)
                    event.accept()
                    return
                else:
                    logger.info(f"Task {task_id} đã ở trong cột {self.title} ({self.status}), không cần di chuyển")
                    event.accept()
                    return
            else:
                logger.warning(f"Không tìm thấy source card cho task {task_id}")
        
        logger.warning("Từ chối thả (không đủ thông tin hoặc không phải task)")
        event.ignore()
...
""")
    ...
    def init_api_client(self):
        ...
        def on_api_ready(api_client):
            ...
            logger.info("API client đã được khởi tạo")
            self.statusBar().showMessage("API client đã sẵn sàng", 3000)
            
        def on_api_error(error_info):
            logger.error(f"Lỗi khởi tạo API client: {error_info[1]}")
    ...
    def load_sample_data(self):
        ...
        def on_data_loaded(data):
            ...
            logger.info("Đã tải dữ liệu mẫu")
    ...
    def move_task(self, task_id, new_status):
        ...
        if task_id not in self.tasks:
            ...
            logger.warning(f"Task {task_id} chưa có trong bộ nhớ, tạo mới với trạng thái {new_status}")
            self.tasks[task_id] = {
                "id": task_id,
        ...
        logger.info(f"Di chuyển task {task_id} từ {old_status} sang {new_status}")
    ...
    def on_move_error(self, error_info):
        ...
        logger.error(f"Lỗi khi di chuyển task: {error_value}")
        ...
        if task_id in self.tasks:
            ...
            if dialog.exec():
                ...
                def on_update_completed(result):
                    ...
                    # Ghi log
                    logger.info(f"Đã cập nhật task {task_id}")
                ...
                def on_update_error(error_info):
                    logger.error(f"Lỗi khi cập nhật task: {error_info[1]}")
...
def main():
    ...
    logger = logging.getLogger('windsurf_memory')
